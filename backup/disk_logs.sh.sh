...
echo "disk_usage: $disk_usage% | free_space: $free_space | date: `date`" >> ~/Desktop/UNIV/backup/logging/space_logs.txt

# Compare with the threshold
if [ "$disk_usage" -ge "$threshold" ]; then
    # Send alert/notification (replace with your notification mechanism)
    echo "Warning: Disk space usage is above $threshold%. Consider freeing up space." >> ~/Desktop/UNIV/backup/logging/space_logs.txt
    echo "-------------------------------------------------------------" >> ~/Desktop/UNIV/backup/logging/space_logs.txt
else
